...
export default function CSVQueryPage() {
  ...
  const [result, setResult] = useState<{ columns: string[]; rows: any[] }>({
    columns: [],
    rows: [],
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isDuplicateSubmission(data)) {
      setAlreadyDisplayedMessage(
        "Results for this query are already displayed."
      );
      return;
    }
    setServerError("");
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-4">
        ...
        <div className="flex flex-col">
          ...
          <input
            ...
            className={`border p-2 rounded mb-1 ${
              inputsErrors.csvUrl ? "border-red-500" : ""
          ...
          <textarea
            ...
            className={`border p-2 rounded min-h-[100px] ${
              inputsErrors.query ? "border-red-500" : ""
            } ${isSubmitting ? "bg-gray-100 cursor-not-allowed" : ""}`}
          />
          {inputsErrors.query && (
      ...
      {!serverError && result.rows.length === 0 && lastSubmittedData && (
        <p className="text-yellow-600 font-medium mt-4">
      ...
      {!serverError && result.rows.length > 0 && (
        <div className="mt-8 w-full flex justify-center">
          <ResultTable
            ...
            onShowMore={() =>
              setVisibleRows((prev) =>
                Math.min(prev + MAX_VISIBLE_ROWS, result.rows.length)
              )
            }
          />
        </div>
